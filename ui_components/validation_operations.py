...
def show_validation_operations(credentials: Dict):
    ...
    # Main tabs - Added Enhanced Validation
...
def show_enhanced_validation(sf_conn):
    ...
    try:
        ...
        if sf_object_info and 'fields' in sf_object_info:
            # Get relevant Salesforce fields (updateable, reference, and picklist fields)
            all_sf_fields = []
            for field in sf_object_info['fields']:
                if (field.get('updateable', True) or 
                    field.get('type') == 'reference' or 
                    field.get('type') in ['picklist', 'multipicklist']):
                    all_sf_fields.append(field)
    ...
    # Step 4: Lookup Resolution (Updated)
    st.write("#### 🔗 Step 4: Lookup Field Resolution")
    
    enable_lookup_resolution = st.checkbox(
        "Enable Lookup Field Resolution",
        value=True,
        help="Automatically resolve lookup fields to Salesforce record IDs"
    )
    ...
    if enable_lookup_resolution:
        try:
            ...
            with st.spinner("🔄 Resolving lookup fields with field mappings..."):
                if field_mappings:
                    # Use enhanced lookup resolution with field mappings
                    lookup_result = resolve_lookup_fields_with_mapping(
                        sf_conn, df_original, selected_object, field_mappings, return_stats=True
                    )
                else:
                    # Fallback to original method if no mappings
                    from ui_components.data_operations import resolve_lookup_fields
                    lookup_result = resolve_lookup_fields(sf_conn, df_original, selected_object, return_stats=True)
                
                if lookup_result is None:
                    st.warning("⚠️ Lookup resolution failed - proceeding with original data")
                    df_transformed = df_original.copy()
                    lookup_fields = {}
                    lookup_count_summary = {}
                elif isinstance(lookup_result, tuple):
                    df_transformed, lookup_fields, lookup_count_summary = lookup_result
                    st.success(f"✅ Lookup resolution completed! {len(lookup_fields)} lookup field(s) processed")
                else:
            ...
            st.warning(f"⚠️ Lookup resolution failed: {str(e)}")
    ...
    # Step 5: Enhanced Transform Validation
    st.write("#### ⚡ Step 5: Enhanced Transform Validation")
    ...
    if st.button("🚀 Start Enhanced Validation", type="primary", use_container_width=True):
        try:
            ...
            # Step 6: Auto-Save Results with New File Management
            st.write("#### 💾 Step 6: Validation Results")
    ...
    # Step 6: Transformation Export
    st.write("---")
    st.write("#### 🔄 Step 6: Export for Transformation")
    ...
    # Step 7: Import Transformed Data
    st.write("---")
    st.write("#### 📥 Step 7: Import Transformed Data")
    ...
    with st.expander("❓ Help - Enhanced Validation Features", expanded=False):
        ...
        **🔍 Validation Criteria:**
        - **Success:** All validations passed (lookup fields transformed, picklist values valid, unique fields unique)
        - **Failure:** One or more issues detected:
          - Lookup fields unchanged from original data
          - Invalid picklist API names
        ...
        **📊 Comprehensive Reporting:**
        ...
        - Statistical breakdown by failure type
        - Field mapping summary and statistics
        - Structured file output for audit trails
        
        **💾 File Organization:**
        - Success data: Clean records ready for Salesforce upload
        - Failure data: Records with issues and detailed descriptions
        - Summary report: Complete analysis with statistics
        - Organized folder structure by org and object
        
        **💡 Pro Tips:**
        - Use the auto-suggestion feature to quickly map similar field names
        - Ensure picklist fields in your data contain API names, not labels
        - Map at least one lookup or picklist field for meaningful validation
        - Review field mapping summary before proceeding to validation
        """)
