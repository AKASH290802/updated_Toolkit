...
def validate_Test_Rule(df):
    """
    Validation Rule: Test_Rule
    ...
    try:
        ...
        if missing_columns:
            print(f"Warning: Missing columns for validation rule 'Test_Rule': {missing_columns}")
        ...
        if len(df) > 0:
            ...
            print(f"DEBUG - Rule 'Test_Rule': Formula result for first record = {sample_value} (True=Error/Invalid, False=NoError/Valid)")
            ...
            if '_is_blank' in str(validation_result):
                ...
                print(f"DEBUG - Rule 'Test_Rule': Field 'Name' value for first record = '{sample_field_value}'")
                print(f"DEBUG - Rule 'Test_Rule': Available columns = {list(df.columns)}")
                if 'Name' not in df.columns:
                    print(f"WARNING - Rule 'Test_Rule': Column 'Name' not found in data! This will cause validation to fail.")
        
        return ~validation_result
        
    except KeyError as e:
        print(f"Warning: Column {e} not found in DataFrame for validation rule 'Test_Rule'")
        return pd.Series([False] * len(df))  # Mark all as invalid if column missing
    except Exception as e:
        print(f"Warning: Error in validation rule 'Test_Rule': {e}")
...
def validate_record(row):
    ...
    is_valid = True
    try:
        print(f"DEBUG validate_record: Calling validate_Test_Rule on row data")
        func_result = validate_Test_Rule(df)
        if hasattr(func_result, 'iloc'):
            rule_results['validate_Test_Rule'] = bool(func_result.iloc[0])
        else:
            rule_results['validate_Test_Rule'] = bool(func_result)
        print(f"DEBUG validate_record: validate_Test_Rule returned {rule_results['validate_Test_Rule']}")
        if not rule_results['validate_Test_Rule']:
            errors.append('validate_Test_Rule')
            print(f"DEBUG validate_record: Added validate_Test_Rule to errors list")
    except Exception as e:
        print(f"ERROR validate_record: validate_Test_Rule failed with error: {str(e)}")
        rule_results['validate_Test_Rule'] = False
        errors.append(f'validate_Test_Rule (error: {str(e)})')
    if errors:
        is_valid = False
