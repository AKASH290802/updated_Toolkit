...
def validate_Account_Name_Mandatory(df):
    """
    Validation Rule: Account_Name_Mandatory
    Salesforce Object: Account
    Primary Field: Name
    
    Error Message: Account Name is mandatory and must be provided before saving the record
    ...
    try:
        ...
        if missing_columns:
            print(f"Warning: Missing columns for validation rule 'Account_Name_Mandatory': {missing_columns}")
        ...
        if len(df) > 0:
            ...
            print(f"DEBUG - Rule 'Account_Name_Mandatory': Formula result for first record = {sample_value} (True=Error/Invalid, False=NoError/Valid)")
            ...
            if '_is_blank' in str(validation_result):
                ...
                print(f"DEBUG - Rule 'Account_Name_Mandatory': Field 'Name' value for first record = '{sample_field_value}'")
                print(f"DEBUG - Rule 'Account_Name_Mandatory': Available columns = {list(df.columns)}")
                if 'Name' not in df.columns:
                    print(f"WARNING - Rule 'Account_Name_Mandatory': Column 'Name' not found in data! This will cause validation to fail.")
        
        return ~validation_result
        
    except KeyError as e:
        print(f"Warning: Column {e} not found in DataFrame for validation rule 'Account_Name_Mandatory'")
        return pd.Series([False] * len(df))  # Mark all as invalid if column missing
    except Exception as e:
        print(f"Warning: Error in validation rule 'Account_Name_Mandatory': {e}")
        return pd.Series([False] * len(df))  # Mark all as invalid on error

# Error message mapping for validation functions
ERROR_MESSAGES = {'validate_Account_Name_Mandatory': 'Account Name is mandatory and must be provided before saving the record'}
...
def validate_record(row):
    ...
    is_valid = True
    try:
        print(f"DEBUG validate_record: Calling validate_Account_Name_Mandatory on row data")
        func_result = validate_Account_Name_Mandatory(df)
        if hasattr(func_result, 'iloc'):
            rule_results['validate_Account_Name_Mandatory'] = bool(func_result.iloc[0])
        else:
            rule_results['validate_Account_Name_Mandatory'] = bool(func_result)
        print(f"DEBUG validate_record: validate_Account_Name_Mandatory returned {rule_results['validate_Account_Name_Mandatory']}")
        if not rule_results['validate_Account_Name_Mandatory']:
            error_msg = ERROR_MESSAGES.get('validate_Account_Name_Mandatory', 'Validation failed for validate_Account_Name_Mandatory')
            errors.append(error_msg)
            print(f"DEBUG validate_record: Added validate_Account_Name_Mandatory to errors list with message: {error_msg}")
    except Exception as e:
        print(f"ERROR validate_record: validate_Account_Name_Mandatory failed with error: {str(e)}")
        rule_results['validate_Account_Name_Mandatory'] = False
        error_msg = ERROR_MESSAGES.get('validate_Account_Name_Mandatory', 'Validation failed for validate_Account_Name_Mandatory')
